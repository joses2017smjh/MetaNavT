...
|---|---:|---:|---:|---:|---:|---:|
| dense_only | 0.272 [0.226, 0.322] | 0.480 [0.408, 0.553] | 0.915 | 0.742 | 45.2 | 0.233 |
| bm25_only | 0.505 [0.454, 0.555] | 0.730 [0.664, 0.793] | 0.843 | 0.587 | 35.8 | 0.462 |
| hybrid | 0.454 [0.402, 0.511] | 0.662 [0.600, 0.727] | 0.938 | 0.751 | 45.8 | 0.430 |
| hybrid+rerank | 0.452 [0.412, 0.496] | 0.747 [0.690, 0.803] | 0.938 | 0.751 | 45.8 | 0.391 |
| hybrid+rerank+router | 0.456 [0.415, 0.502] | 0.745 [0.689, 0.801] | 0.938 | 0.714 | 43.5 | 0.402 |
| hybrid+rerank+router+staleness | 0.493 [0.452, 0.538] | 0.748 [0.691, 0.804] | 0.938 | 0.662 | 40.4 | 0.464 |
...
- `+ overlap rerank`: the fallback reranker, a loser on aggregate nDCG@10 until `bge-reranker-v2-m3` is measured (M2).
- `+ router`: skips embedding and reranking on exact-path queries; the gain is on that slice only and rests on n = 8.
- `+ staleness T1`: prefers the current version of a file; the one row whose paired delta over the previous row excludes zero.
...
make bench-table      # renders the blocks above from main.json
...
**The published rerank row is the overlap teacher**, not `BAAI/bge-reranker-v2-m3`. Overlap lost to plain RRF on aggregate nDCG@10 (see the [ablation table](#the-number)). `make bench-frontier` will load bge-m3 when the weights are local (`reranker_loaded`); otherwise it records `reranker_fallback=overlap`.
